...
def entry_parser(feed_urls):
    ...
    time=today_base.strftime("%H00")
     for feed_url in feed_urls:
         html=urllib.request.urlopen(feed_url)
         soup = BeautifulSoup(html)
         containers = soup.find_all("div",class_="rankTtl")
         ent_text =""
         ...
         for key, value in histogram.most_common(): #頻出単語集計
                ...
                r.zincrby("twitrend_urls/"+str(today),str(key),int(value)) #当日全URLサイト
                r.zincrby("twitrend_urls/"+str(today)+"/"+str(time),str(key),int(value)) #当日全URLサイト1時間ごと
         #sleep(3)
         #sleep( 2 )
    return "hoge"
